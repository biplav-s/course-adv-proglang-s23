...
void demoRelationalOperators() {
	...
	int a = 5;
	float b = 5.0;
	cout << "Checking = on (" << a << "," << b << "). Result is: " << (a == b)
			<< endl;
...
void demoBitOperators() {
	...
	cout << "Checking '&' on (" << oct << c << "," << oct << d
			<< "). Result is: " << oct << (c & d) << endl;

	cout << "Checking '~' on (" << c << "). Result is: " << oct << (~c) << endl;

	cout << "Checking '|' on (" << c << "," << d << "). Result is: " << oct
			<< (c | d) << endl;
...
void demoStrings() {
	...
	cout << "String:" << s << ", oper:capacity, result:" << s.capacity()
			<< endl;
	cout << "String:" << s << ", oper:substr(20,24), result:"
			<< s.substr(20, 24) << endl;
...
void demoMaths() {
	...
	float n = 27;
	std::cout << "Maths: oper:sqrt (sq root) of n:" << n << ", result:"
			<< sqrt(n) << endl;
	std::cout << "Maths: oper:cqrt (cubic root) of n:" << n << ", result:"
			<< cbrt(n) << endl;

	n = 27.498;
	std::cout << "Maths: oper:round,  n:" << n << ", result:" << round(n)
			<< endl;
	std::cout << "Maths: oper:nearby int,  n:" << n << ", result:"
			<< nearbyint(n) << endl;
	std::cout << "Maths: oper:nearbyint with n - 0.5,  n:" << n << ", result:"
			<< nearbyint(n - 0.5) << endl;
	std::cout << "Maths: oper:nearbyint with n + 0.5,  n:" << n << ", result:"
			<< nearbyint(n + 0.5) << endl;

	std::cout << "Maths: oper:nearbyint,  n:" << -INFINITY << ", result:"
			<< nearbyint(-INFINITY) << endl;
	std::cout << "Maths: oper:nearbyint,  n:" << NAN << ", result:"
			<< nearbyint(NAN) << endl;
...
// Credits: https://en.cppreference.com/w/cpp/language/operators
// Note: in case of compilation error, use option -std=c++11
//       See: https://stackoverflow.com/questions/27924050/expected-at-end-of-declaration-vector-c
...
void demoCustomOperator() {

	// f and g are objects that can be used like a function.
	Linear f { 2, 10 };
	// Represents function 2x + 1.

	double f_0 = f(0);
	cout << "\nDEMO: Initializing f: a * x + b with  (a = 2, b = 10)" << endl;
	cout << "\nDEMO: Value of function at f_0 - .. 2x + 1 is - " << f_0 << endl;
	double f_1 = f(1);
	cout << "\nDEMO: Value of function at f_1 - .. 2x + 1 is -  " << f_1 << endl;

	cout << "\nDEMO: Initializing f: a * x + b with  (a = -1, b = 0)" << endl;
	Linear g { -1, 0 };
	// Represents function -x.
	double g_100 = g(100);
	cout << "\nDEMO: Custom function .. g_100 of -x is - " << g_100 << endl;
...
void demoAlgos() {
	...
	string s = s_orig;
	sort(s.begin(), s.end());
	cout << "Algos: permutation/sort of s:" << s_orig << ", result:" << s
			<< endl;

	while (next_permutation(s.begin(), s.end())) {
		cout << "Algos: permutation of s:" << s_orig << ", result:" << s
				<< endl;
	}
